...
As the first sub-second ZK prover, the protocol can execute analytic queries over 1,000,000-row tables in less than a second (see benchmarks below). It can aggregate over millions of rows of indexed data within Ethereum block time on a single NVIDIA T4. The protocol is designed to support both onchain and offchain verification, leveraging a novel commitment scheme which significantly lowers gas fees with onchain verification.
