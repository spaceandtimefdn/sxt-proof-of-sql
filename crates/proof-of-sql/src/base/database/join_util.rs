...
pub(crate) fn get_sort_merge_join_indexes<'a, S: Scalar>(
    ...
    while left_pos < left_indexes.len() && right_pos < right_indexes.len() {
        ...
        let Ok(comparison_result) =
            compare_single_row_of_tables(left_on, right_on, left_idx, right_idx)
        else {
            return Vec::new();
        };
        ...
        match comparison_result {
            ...
            Ordering::Equal => {
                ...
                while right_end < right_indexes.len()
                    && compare_indexes_by_columns(right_on, right_idx, right_indexes[right_end])
                        == Ordering::Equal
                {
                    right_end += 1;
                }
