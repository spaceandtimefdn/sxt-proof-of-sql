        ...
        match value {
                ...
                *scale,
                decimals
                    ...
                    .map(|&d| {
                        S::from(d)
                            .to_limbs_opt()
                            .expect("limb conversion required")
                    })
                    .collect(),
            ),
            ...
            OwnedColumn::VarChar(strings) => CommittableColumn::VarChar(
                strings
                    .iter()
                    .map(|s| {
                        S::from_str_via_hash_opt(s)
                            .expect("hashing conversion required")
                            .to_limbs_opt()
                            .expect("limb conversion required")
                    })
                    .collect(),
            ...
            OwnedColumn::VarBinary(bytes) => CommittableColumn::VarBinary(
                bytes
                    .iter()
                    .map(|b| {
                        S::from_byte_slice_via_hash_opt(b)
                            .expect("hashing conversion required")
                            .to_limbs_opt()
                            .expect("limb conversion required")
                    })
