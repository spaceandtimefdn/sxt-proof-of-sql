...
mod time_unit_tests {
    ...
    #[test]
    fn test_precision_scale_conversion() {
    ...
    #[test]
    fn test_invalid_precision_keeps_original_error_value() {
        let result = PoSQLTimeUnit::try_from("cat");
        assert!(matches!(
            result,
            Err(PoSQLTimestampError::UnsupportedPrecision { error }) if error == "cat"
        ));
    }
}
