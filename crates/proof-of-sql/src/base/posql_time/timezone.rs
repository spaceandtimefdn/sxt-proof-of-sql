...
mod timezone_parsing_tests {
    ...
    #[test]
    fn we_can_parse_utc_aliases_case_insensitively() {
        for timezone_as_str in ["Z", "UTC", "utc", "00:00", "+00:00", "0:00", "+0:00"] {
            let timezone =
                PoSQLTimeZone::try_from(&Some(Arc::<str>::from(timezone_as_str))).unwrap();
    ...
    fn we_cannot_parse_invalid_time_zone_offset() {
        for timezone_as_str in ["+0a:00", "+01:0x"] {
            let timezone_err =
                PoSQLTimeZone::try_from(&Some(Arc::<str>::from(timezone_as_str))).unwrap_err();
            assert!(matches!(
                timezone_err,
