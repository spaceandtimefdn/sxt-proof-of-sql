...
mod batch_to_columns_tests {
    ...
    #[test]
    fn batch_to_columns_preserves_arrow_field_order_names_and_values() {
        ...
        let batch = RecordBatch::try_new(
            schema,
            vec![
                Arc::new(Int64Array::from(vec![10, 20, -5])) as ArrayRef,
        ...
        match &columns[0].1 {
            Column::BigInt(values) => assert_eq!(*values, [10, 20, -5]),
            unexpected => panic!("expected BigInt column, got {unexpected:?}"),
        }

        assert_eq!(columns[1].0.value, "is_active");
        match &columns[1].1 {
            Column::Boolean(values) => assert_eq!(*values, [true, false, true]),
            unexpected => panic!("expected Boolean column, got {unexpected:?}"),
        }
    }

    #[test]
    fn batch_to_columns_propagates_arrow_array_conversion_errors() {
        ...
        let error = batch_to_columns::<TestScalar>(&batch, &alloc).unwrap_err();
