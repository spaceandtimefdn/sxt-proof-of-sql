use super::{final_round_evaluate_sign, verifier_evaluate_sign};
use crate::{
    base::{
        database::{try_divide_modulo_column_types, Column, ColumnType, LiteralValue, Table},
        map::IndexMap,
...
impl DivideAndModuloExpr {
    ...
    fn final_round_evaluate_base<'a, S: Scalar, U: DivideAndModuloExprUtilities<S>>(
        ...
        let remainder_minus_rhs = add_subtract_columns(remainder, rhs_column, alloc, true);
        let remainder_plus_rhs = add_subtract_columns(remainder, rhs_column, alloc, false);
        let rhs_as_scalars = rhs_column.to_scalar();
        ...
        // (q + MIN) * v - (q' - MIN) = 0 where v = (q' - MIN) / (q + MIN) if q is not - MIN
        // Ensures q = -MIN only if q' = MIN
        let quotient_wrapped_slice =
            alloc.alloc_slice_fill_iter(quotient_wrapped.to_scalar()) as &[_];
        let v = alloc.alloc_slice_fill_iter(quotient.iter().zip(quotient_wrapped_slice).map(
            |(q, q_prime)| (*q + min_scalar).inv().unwrap_or(S::ONE) * (*q_prime - min_scalar),
        ));
        let v = Column::Scalar(v);
        ...
        builder.produce_sumcheck_subpolynomial(
            ...
            vec![
                (
                    S::one(),
                    vec![
                        Box::new(quotient_wrapped_slice),
                        Box::new(quotient),
                        Box::new(v),
                    ],
                ),
                (S::one(), vec![Box::new(min_scalar_column)]),
                (-S::one(), vec![Box::new(quotient_wrapped_slice)]),
                (
                    -S::one(),
                    vec![
        ...
        final_round_evaluate_sign(
            builder,
            alloc,
            add_subtract_columns(neg_min_sqrt_scalar_column, s, alloc, false),
        );
        final_round_evaluate_sign(
            builder,
            alloc,
            add_subtract_columns(neg_min_sqrt_scalar_column, s, alloc, true),
        );

        // MIN < q' < MAX
        // Confirms that q is not too big.
        final_round_evaluate_sign(builder, alloc, quotient_wrapped_slice);

        // sign(a) * r = sign(r) * r and sign(r - b) * b + sign(r - b) - b = 0
        // constrains remainder to be in the correct range
        let lhs_sign = final_round_evaluate_sign(
            builder,
            alloc,
            alloc.alloc_slice_fill_iter(lhs_column.to_scalar()) as &[_],
        );
        let remainder_sign = final_round_evaluate_sign(
            builder,
            alloc,
            alloc.alloc_slice_fill_iter(remainder.to_scalar()) as &[_],
        );

        let remainder_minus_rhs_sign =
            final_round_evaluate_sign(builder, alloc, remainder_minus_rhs);
        let remainder_plus_rhs_sign = final_round_evaluate_sign(builder, alloc, remainder_plus_rhs);
    ...
    fn verifier_evaluate<S: Scalar, B: VerificationBuilder<S>>(
        ...
        let u = builder.try_consume_final_round_mle_evaluation()?;
        let v = builder.try_consume_final_round_mle_evaluation()?;

        // For the following constraints, the comments shall denote the vraiables as follows
        // q: quotient
        // q': quotient_wrapped
        // r: remainder
        // a: lhs
        ...
        builder.try_produce_sumcheck_subpolynomial_evaluation(
            SumcheckSubpolynomialType::Identity,
            (quotient + min_eval) * v - quotient_wrapped + min_eval,
            3,
        )?;
        ...
        // -2^b <= q' < 2^b, where b is
        verifier_evaluate_sign(
            builder,
            quotient_wrapped,
            one_eval,
            Some(
                (self.data_type().0.to_integer_bits().unwrap())
                    .try_into()
                    .unwrap(),
            ),
        )?;
...
mod tests {
    ...
    fn default_divide_columns(lhs: &[i128], rhs: &[i128]) -> (Vec<TestScalar>, Vec<TestScalar>) {
        ...
        (quotient_wrapped.to_scalar(), quotient.to_vec())
    }
    ...
    fn check_constraints(
        ...
        let first_round_builder = FirstRoundBuilder::new(lhs.len());
        let mut final_round_builder = FinalRoundBuilder::new(lhs.len(), VecDeque::new());
        let table = Table::try_new(indexmap! {
            lhs_ident.clone() => Column::Int128::<TestScalar>(lhs),
            rhs_ident.clone() => Column::Int128::<TestScalar>(rhs),
        })
        .unwrap();
        ...
        let mock_verification_builder = run_verify_for_each_row(
            ...
            |verification_builder, chi_eval, evaluation_point| {
                let accessor = indexmap! {
                    lhs_ident.clone() => lhs.inner_product(evaluation_point),
                    rhs_ident.clone() => rhs.inner_product(evaluation_point)
                };
                divide_and_modulo_expr
    ...
    /// A malicious prover can try to craft a scenario where the quotient and denominator are so big that their product
    /// exceeds the max value of the `Scalar` type. This can satisfy the division algorithm and allow for tampering
    /// of both the quotient and the remainder. The verifier checks to see that at least one of the quotient and
    /// denominator is small enough that this doesn't happen.
    #[test]
