...
impl ProofPlan for GroupByExec {
    fn verifier_evaluate<S: Scalar>(
        ...
        let (g_in_star_eval, _) =
            fold_gadget.verify_evaluate(builder, &group_by_evals, input_chi_eval)?;
...
impl ProverEvaluate for GroupByExec {
    ...
    fn final_round_evaluate<'a, S: Scalar>(
        ...
        let group_by_columns = self
            ...
            .collect::<PlaceholderResult<Vec<_>>>()?;
        let fold_gadget = FoldLogExpr::new(alpha, beta);
        let (g_in_star, _) = fold_gadget.final_round_evaluate(builder, alloc, &group_by_columns, n);
        // End compute g_in_star
