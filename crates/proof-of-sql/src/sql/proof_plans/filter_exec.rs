...
mod tests {
    ...
    #[test]
    fn we_can_read_filter_metadata_without_blitzar() {
        let table_ref = table_ref();
        let plan = plan(&table_ref);
        let input = DynProofPlan::new_table(table_ref.clone(), schema());
    ...
    #[test]
    fn we_can_evaluate_filter_rounds_without_blitzar() {
        let alloc = Bump::new();
        let table_ref = table_ref();
