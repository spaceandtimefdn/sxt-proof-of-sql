use crate::{
    ...
    sql::{
        ...
        proof_exprs::{test_utility::*, DynProofExpr, NegExpr},
...
#[test]
fn we_can_prove_a_query_with_negation() {
    ...
    let ast = projection(
        vec![
            aliased_plan(
                neg(subtract(column(&t, "a", &accessor), const_bigint(2))),
                "a",
            ),
...
#[test]
fn we_cannot_construct_neg_expr_with_bad_type() {
    let input_expr = Box::new(DynProofExpr::new_literal(LiteralValue::VarChar(
        "test".to_string(),
    )));
    let err = NegExpr::try_new(input_expr).unwrap_err();
    assert!(
