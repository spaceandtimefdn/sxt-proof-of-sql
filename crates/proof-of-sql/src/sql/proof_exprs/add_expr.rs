...
mod tests {
    ...
    #[test]
    fn we_can_verify_add_expr_helper_paths() {
        let alloc = Bump::new();
        let lhs = [
            TestScalar::from(2),
            TestScalar::from(7),
        ...
        assert_eq!(add_expr.lhs().data_type(), decimal_type);
        assert_eq!(add_expr.rhs().data_type(), decimal_type);

        let mut referenced_columns: IndexSet<ColumnRef> = IndexSet::default();
        add_expr.get_column_references(&mut referenced_columns);
        assert_eq!(
