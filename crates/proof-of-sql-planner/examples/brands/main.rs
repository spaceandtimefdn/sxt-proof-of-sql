...
//! you can run `cargo run --release --example brands --no-default-features --features="cpu-perf"` instead. It will be slower for proof generation.
use datafusion::{
    arrow::{
...
fn main() {
    ...
    accessor.add_table(
        TableRef::from_names(None, "global_brands"),
        OwnedTable::try_from(brands_batch).unwrap(),
        0,
    );

    // Query 1: List the names of brands founded before 1950
    ...
    // Query 2: List countries with total revenue grouped by country
    prove_and_verify_query(
        "SELECT Country, SUM(Revenue) AS total_revenue, COUNT(1) AS num_brands FROM global_brands GROUP BY Country",
        &accessor,
        &prover_setup,
        &verifier_setup,
    );
