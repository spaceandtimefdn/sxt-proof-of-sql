...
//! you can run `cargo run --release --example countries --no-default-features --features="cpu-perf"` instead. It will be slower for proof generation.
use datafusion::{
    arrow::{
...
fn main() {
    ...
    accessor.add_table(
        TableRef::from_names(None, "countries"),
    ...
    prove_and_verify_query(
        "SELECT SUM(gdp) AS total_market_cap, COUNT(1) FROM countries WHERE country = 'China' OR country = 'India'",
        &accessor,
        &prover_setup,
        &verifier_setup,
    );
