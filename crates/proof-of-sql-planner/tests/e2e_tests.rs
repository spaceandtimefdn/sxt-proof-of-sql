...
/// Keep planner end-to-end Dory setup generation sized to the tiny tables used here.
///
/// The tests in this file exercise query planning/proving behavior with at most a
/// handful of rows, so a `max_nu` of 3 (8 rows) preserves coverage while avoiding
/// the larger setup generated by the previous `max_nu` of 5 (32 rows) in every
/// test case.
const PLANNER_E2E_TEST_MAX_NU: usize = 3;
...
fn test_empty_sql() {
    ...
    let public_parameters = PublicParameters::test_rand(PLANNER_E2E_TEST_MAX_NU, &mut test_rng());
