...
As the first sub-second ZK prover, the protocol can execute analytic queries over 1M+ rows in less than a second (see benchmarks below). It can aggregate over millions of rows of indexed data within Ethereum block time on a single NVIDIA T4. The protocol is designed to support both onchain and offchain verification, leveraging a novel commitment scheme which significantly lowers gas fees with onchain verification.
...
<p align="center"><img src="https://raw.githubusercontent.com/spaceandtimelabs/sxt-proof-of-sql/main/docs/HyperKZG_A100_200k_2025.07.22.png" alt="Proof Of SQL Benchmarks (200k - A100)" width="50%"/><img src="https://raw.githubusercontent.com/spaceandtimelabs/sxt-proof-of-sql/main/docs/HyperKZG_multi_A100_200k_2025.07.22.png" alt="Proof Of SQL Benchmarks (200k - Multi-A100)" width="50%"/><img src="https://raw.githubusercontent.com/spaceandtimelabs/sxt-proof-of-sql/main/docs/HyperKZG_A100_2025.07.22.png" alt="Proof Of SQL Benchmarks (10m - A100)" width="50%"/><img src="https://raw.githubusercontent.com/spaceandtimelabs/sxt-proof-of-sql/main/docs/HyperKZG_multi_A100_2025.07.22.png" alt="Proof Of SQL Benchmarks (10m - Multi-A100)" width="50%"/></p>
...
To ensure a smooth development experience and rapid feedback cycles, the test suite for Proof of SQL has been optimized for performance.

### Running Tests

You can run all tests using `cargo nextest run --all-features`. For faster local development, it's recommended to leverage the built-in caching mechanism for Dory setups.

### Test Performance and Caching

A significant portion of the test suite's runtime previously stemmed from the generation of Dory `PublicParameters`, `ProverSetup`, and `VerifierSetup` objects. To mitigate this, these computationally intensive setups are now cached.

Upon the first test run, these setups are generated and saved to a designated location (e.g., `crates/proof-of-sql-planner/test_assets/dory_setups/`). Subsequent test runs will load these pre-computed setups, drastically reducing test execution time.

If you need to regenerate the cached setups (e.g., due to changes in the underlying Dory library or test parameters), you can delete the cached files, and they will be automatically re-generated on the next test run. This ensures that tests always run against the correct setup while maintaining performance benefits.

This caching strategy significantly reduces the overall CI runtime and improves the local development workflow, allowing contributors to iterate faster.
