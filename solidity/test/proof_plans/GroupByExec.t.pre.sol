...
contract GroupByExecTest is Test {
    ...
    function testGroupByExec() public pure {
        bytes memory plan = abi.encodePacked(
            uint64(0), // table_number
            uint64(3), // total_column_count
            uint64(1), // group_by_count
        ...
        builder.finalRoundMLEs = new uint256[](36); // 9 mles times 4 rows
        {
            ...
            for (uint8 i = 0; i < 4; ++i) {
                ...
                builder.finalRoundMLEs[i * 9 + 3] = sumOut[i];
                builder.finalRoundMLEs[i * 9 + 4] = count[i];
                // Monotonicity check
                builder.finalRoundMLEs[i * 9 + 5] = shiftedGOut[i];
                builder.finalRoundMLEs[i * 9 + 6] = cStarEval[i];
                builder.finalRoundMLEs[i * 9 + 7] = dStarEval[i];
                builder.finalRoundMLEs[i * 9 + 8] = sign[i];
            }
        }
