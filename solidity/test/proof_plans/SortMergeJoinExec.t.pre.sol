...
contract SortMergeJoinExecTest is Test {
    ...
    uint256 private constant _FINAL_ROUND_MLE_TOTAL = 21;
    uint256 private constant _BIT_DITRIBUTION_TOTAL = 2;
    uint256 private constant _ROW_TOTAL = 4;
    uint256 private constant _VARYING_BIT_TOTAL = 67;
    ...
    function testSortMergeJoinExec() public pure {
        ...
        {
            uint256[_CHI_TOTAL] memory lengths = [uint256(3), 2, 3, 4];
            for (uint8 i = 0; i < _ROW_TOTAL; ++i) {
        ...
        {
            uint256[_RHO_TOTAL] memory lengths = [uint256(2), 3, 3, 2, 3, 4];
            for (uint8 i = 0; i < _ROW_TOTAL; ++i) {
        ...
        builder.finalRoundMLEs = new uint256[](_ROW_TOTAL * (_FINAL_ROUND_MLE_TOTAL + _VARYING_BIT_TOTAL));
        {
            uint256[_FINAL_ROUND_MLE_TOTAL][_ROW_TOTAL] memory finalRoundMLEs = [
                [
            ...
            uint256[5][_ROW_TOTAL] memory bitEvals =
                [[uint256(1), 1, 1, 1, 0], [uint256(1), 0, 1, 1, 0], [uint256(1), 1, 0, 1, 0], [uint256(1), 0, 0, 1, 1]];

            for (uint8 i = 0; i < _ROW_TOTAL; ++i) {
                for (uint8 j = 0; j < _FINAL_ROUND_MLE_TOTAL; ++j) {
                    builder.finalRoundMLEs[i * (_FINAL_ROUND_MLE_TOTAL + _VARYING_BIT_TOTAL) + j] = finalRoundMLEs[i][j];
                }
                builder.finalRoundMLEs[i * (_FINAL_ROUND_MLE_TOTAL + _VARYING_BIT_TOTAL) + _FINAL_ROUND_MLE_TOTAL] =
                    bitEvals[i][0];
                for (uint8 j = 1; j < _VARYING_BIT_TOTAL - 3; ++j) {
                    builder.finalRoundMLEs[i * (_FINAL_ROUND_MLE_TOTAL + _VARYING_BIT_TOTAL) + _FINAL_ROUND_MLE_TOTAL
                        + j] = bitEvals[i][1];
                }
                builder.finalRoundMLEs[(i + 1) * (_FINAL_ROUND_MLE_TOTAL + _VARYING_BIT_TOTAL) - 3] = bitEvals[i][2];
                builder.finalRoundMLEs[(i + 1) * (_FINAL_ROUND_MLE_TOTAL + _VARYING_BIT_TOTAL) - 2] = bitEvals[i][3];
                builder.finalRoundMLEs[(i + 1) * (_FINAL_ROUND_MLE_TOTAL + _VARYING_BIT_TOTAL) - 1] = bitEvals[i][4];
            }
        }
