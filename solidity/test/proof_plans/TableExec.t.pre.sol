...
contract TableExecTest is Test {
    ...
    function testFuzzTableExec(
        VerificationBuilder.Builder memory builder,
        uint64 tableRef,
        uint64[] memory columnIndices,
        bytes memory trailingExpr
    ) public pure {
        // Limit column indices to a reasonable size for testing
        vm.assume(columnIndices.length > 0 && columnIndices.length <= 10);
        
        // Create the plan with table_ref and column indices
        bytes memory plan = abi.encodePacked(tableRef, uint64(columnIndices.length));
        for (uint256 i = 0; i < columnIndices.length; i++) {
            plan = abi.encodePacked(plan, columnIndices[i]);
        }
        ...
        // Ensure columnEvaluations array is long enough for our test
        ...
        // Verify results
        assert(evals.length == columnIndices.length);
        for (uint256 i = 0; i < columnIndices.length; i++) {
