...
contract LiteralExprTest is Test {
    function testLiteralExpr() public pure {
        bytes memory exprIn = abi.encodePacked(COLUMN_TYPE_BIGINT_VARIANT, int64(2), hex"abcdef");
    ...
    function testFuzzBigIntLiteralExpr(int64 literalValue, uint256 chiInEval, bytes memory trailingExpr) public pure {
        bytes memory exprIn = abi.encodePacked(COLUMN_TYPE_BIGINT_VARIANT, literalValue, trailingExpr);
    ...
    function testVariantsMatchEnum() public pure {
        assert(uint32(LiteralExpr.LiteralVariant.BigInt) == COLUMN_TYPE_BIGINT_VARIANT);
    }
}
