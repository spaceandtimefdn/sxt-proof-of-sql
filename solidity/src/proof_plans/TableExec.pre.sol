        ...
        )
    {
        ...
        assembly {
            ...
            function table_exec_evaluate(plan_ptr, builder_ptr) -> plan_ptr_out, evaluations_ptr {
                // Get table reference from the plan
                let table_ref := shr(UINT64_PADDING_BITS, calldataload(plan_ptr))
                plan_ptr := add(plan_ptr, UINT64_SIZE)

                // Get the chi evaluation for the table
                let chi_eval := builder_get_table_chi_evaluation(builder_ptr, table_ref)
                ...
                // For each column field in the schema, get evaluations from the accessor
                let accessor_ptr := mload(add(builder_ptr, BUILDER_COLUMN_EVALUATIONS_OFFSET))

                // Read column evaluations for each field in the schema
                for { let i := 0 } lt(i, column_count) { i := add(i, 1) } {
                    ...
                    let column_eval := get_array_element(accessor_ptr, column_num)
