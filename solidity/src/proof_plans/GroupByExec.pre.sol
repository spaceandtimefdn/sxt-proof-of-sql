        ...
        )
    {
        ...
        assembly {
                ...
                evaluations_ptr
            {
                ...
                if not(eq(column_count, 1)) {
                    // Error if group by column count is not 1
                    err(ERR_UNPROVABLE_GROUP_BY)
                }
                ...
                g_out_fold, evaluations_ptr := fold_final_round_mles(builder_ptr, beta, column_count)
                ...
                sum_out_fold, evaluations_ptr := fold_final_round_mles(builder_ptr, beta, column_count)
                sum_in_fold := mulmod_bn254(sum_in_fold, beta)
                sum_out_fold := mulmod_bn254(sum_out_fold, beta)
            ...
            function group_by_exec_evaluate(plan_ptr, builder_ptr) -> plan_ptr_out, evaluations_ptr, output_chi_eval {
                ...
                plan_ptr, g_in_fold, g_out_fold, sum_in_fold, sum_out_fold, evaluations_ptr :=
                    compute_groupby_folds(plan_ptr, builder_ptr, beta, input_chi_eval)
                let g_eval := g_in_fold
                ...
                sum_in_fold := addmod_bn254(sum_in_fold, input_chi_eval)
                sum_out_fold := addmod_bn254(sum_out_fold, count_out_eval)
                ...
                builder_produce_zerosum_constraint(
                    builder_ptr,
                    submod_bn254(
                        mulmod_bn254(mulmod_bn254(g_in_star_eval, selection_eval), sum_in_fold),
                        mulmod_bn254(g_out_star_eval, sum_out_fold)
                    ),
                    3
                )
