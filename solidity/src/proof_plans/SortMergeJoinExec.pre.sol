        ...
        )
    {
        ...
        assembly {
            ...
            function evaluate_and_membership_check_right_column_evals(
                ...
                // We can use i_eval as an indexing value until it's time to consume. This saves a local variable.
                for { i_eval := num_join_columns } i_eval { i_eval := sub(i_eval, 1) } {
                ...
                for { i_eval := sub(res_column_length, num_join_columns) } i_eval { i_eval := sub(i_eval, 1) } {
                    res_column_evals := add(res_column_evals, WORD_SIZE)
                    res_column_evals_out := add(res_column_evals_out, WORD_SIZE)
                ...
                for { i_eval := sub(num_columns, num_join_columns) } i_eval { i_eval := sub(i_eval, 1) } {
                    right_column_evals := add(right_column_evals, WORD_SIZE)
                    res_column_evals_out := add(res_column_evals_out, WORD_SIZE)
            ...
            ) -> plan_ptr_out, res_column_evals, w_eval {
                let join_evals, chi_eval
                plan_ptr, join_evals, chi_eval, res_column_evals :=
                ...
                res_column_evals
            {
                let i_eval, w_eval
                {
                    // We only need u eval and u chi in order to run the checks on the left and right sides
                    ...
                    plan_ptr, res_column_evals, w_eval :=
                    ...
                    let w_right_eval
                    plan_ptr, res_column_evals, i_eval, w_right_eval :=
                    ...
                    w_eval := mulmod_bn254(w_eval, w_right_eval)
                }
