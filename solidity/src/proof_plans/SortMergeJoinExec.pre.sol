...
library SortMergeJoinExec {
        ...
        )
    {
        ...
        assembly {
            ...
            function temp(builder_ptr, num_join_columns, hat_evals, res_chi_eval, chi_eval, left_right_and_output_evaluations) ->
                i_eval,
                right_column_evals
            {
            ...
            function evaluate_and_membership_check_right_column_evals(
                builder_ptr, alpha_and_beta, num_join_columns, hat_evals, res_chi_eval, chi_eval, left_right_and_output_evaluations
            ) -> i_eval {
                let right_column_evals
                i_eval, right_column_evals :=
                    temp(builder_ptr, num_join_columns, hat_evals, res_chi_eval, chi_eval, left_right_and_output_evaluations)
            ...
            function evaluate_consume_and_check_left_column_evals(
                ...
                plan_ptr_out, hat_evals, join_evals := evaluate_input_plans(plan_ptr, builder_ptr, mload(left_right_and_output_evaluations))
            ...
            function evaluate_and_check_right_join_evals(
                ...
                plan_ptr, hat_evals, join_evals := evaluate_input_plans(plan_ptr, builder_ptr, mload(add(left_right_and_output_evaluations, WORD_SIZE)))
                i_eval :=
                    evaluate_and_membership_check_right_column_evals(
                        builder_ptr, alpha_and_beta, mload(join_evals), hat_evals, res_chi_eval, chi_eval, left_right_and_output_evaluations
                    )
            ...
            function run_checks(
                ...
                plan_ptr :=
                    evaluate_consume_and_check_left_column_evals(
                        plan_ptr, builder_ptr, alpha_and_beta, output_chi_eval, left_right_and_output_evaluations, left_chi_eval
                    )
                plan_ptr :=
                    evaluate_and_check_right_join_evals(
                        plan_ptr,
                        builder_ptr,
                        alpha_and_beta,
                        output_chi_eval,
                ...
                output_chi_eval
            {
                ...
                {
                    let left_evaluations
                    plan_ptr, left_evaluations, output_length, left_chi_eval := proof_plan_evaluate(plan_ptr, builder_ptr)
                    mstore(left_right_and_output_evaluations, left_evaluations)
                }
                {
                    let right_evaluations
                    plan_ptr, right_evaluations, output_length, right_chi_eval := proof_plan_evaluate(plan_ptr, builder_ptr)
