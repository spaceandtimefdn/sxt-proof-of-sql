        ...
        )
    {
        ...
        assembly {
            ...
            function get_and_verify_slice_length(plan_ptr, builder_ptr, input_length) ->
                ...
                selection_eval
            {
                let skip := shr(UINT64_PADDING_BITS, calldataload(plan_ptr))
                plan_ptr := add(plan_ptr, UINT64_SIZE)
                output_length, output_chi_eval := builder_consume_chi_evaluation_with_length(builder_ptr)
                let offset
                offset, selection_eval := builder_consume_chi_evaluation_with_length(builder_ptr)
                if sub(min(skip, input_length), offset) { err(ERR_SLICE_OFFSET_PLAN_VALUE_MISMATCH) }
                let max_length, max_eval := builder_consume_chi_evaluation_with_length(builder_ptr)
                if sub(max_length, add(offset, output_length)) { err(ERR_SLICE_OFFSET_SELECTION_SIZE_MISMATCH) }
                selection_eval := submod_bn254(max_eval, selection_eval)
                {
                    let is_populated := shr(BOOLEAN_PADDING_BITS, calldataload(plan_ptr)) // skip `fetch` field
                    plan_ptr := add(plan_ptr, BOOLEAN_SIZE)
                    switch is_populated
                    case 0 { if sub(max_length, input_length) { err(ERR_SLICE_MAX_LENGTH_MISMATCH) } }
                    default {
                        let fetch := shr(UINT64_PADDING_BITS, calldataload(plan_ptr))
                        if sub(max_length, min(add(skip, fetch), input_length)) { err(ERR_SLICE_MAX_LENGTH_MISMATCH) }
                        plan_ptr := add(plan_ptr, UINT64_SIZE)
                    }
                }
