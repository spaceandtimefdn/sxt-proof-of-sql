...
library MembershipCheck {
    ...
    /// * `alpha` - a challenge
    /// * `beta` - a challenge
    /// * `chi_n_eval` - the evaluation of a column of 1s with length n, where n is the number of rows that should contain the relevant candidate rows
    /// * `chi_m_eval` - the evaluation of a column of 1s with length m, where m is the number of rows that should be contained by the relevant column rows
    ...
    /// @param __chiNEval The evaluation of a column of 1s with length n, where n is the number of rows that should contain the relevant candidate rows
    /// @param __chiMEval The evaluation of a column of 1s with length m, where m is the number of rows that should be contained by the relevant column rows
    ...
    function __membershipCheckEvaluate( // solhint-disable-line gas-calldata-parameters
        ...
        assembly {
            ...
            function membership_check_evaluate(
                ...
                if sub(num_columns, num_candidate_columns) { err(ERR_MISMATCHING_COLUMN_AND_CANDIDATE_LENGTHS) }
