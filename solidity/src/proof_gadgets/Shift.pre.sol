...
library Shift {
    ...
    function __shiftEvaluate( // solhint-disable-line gas-calldata-parameters
        ...
        assembly {
            ...
            function get_shift_identity_constraint(star, chi_plus_one, fold) -> constraint {
                constraint := addmod_bn254(submod_bn254(star, chi_plus_one), mulmod_bn254(fold, star))
            }

            function get_fold(alpha, beta, eval, rho) -> fold {
            ...
            function shift_evaluate(builder_ptr, alpha, beta, expr_eval, shifted_expr_eval, chi_eval, chi_plus_one_eval)
            {
                ...
                builder_produce_identity_constraint(
                    builder_ptr,
                    get_shift_identity_constraint(
                        c_star_eval,
                        chi_plus_one_eval,
                        get_fold(alpha, beta, expr_eval, addmod_bn254(rho_eval, chi_eval))
                    ),
                    2
                )
                builder_produce_identity_constraint(
                    builder_ptr,
                    get_shift_identity_constraint(
                        d_star_eval, chi_plus_one_eval, get_fold(alpha, beta, shifted_expr_eval, rho_plus_one_eval)
                    ),
                    2
                )
            }
