...
library ProofOfSqlTable {
    ...
    /// @dev Deserializes a column into a scalar column.
    /// @param __table The table
    /// @param __index The index of the column to be deserialized
    /// @return __scalarColumn The deserialized scalar column
    function readScalarColumn(Table memory __table, uint256 __index)
        internal
        pure
        returns (uint256[] memory __scalarColumn)
    {
        Column memory __column = __table.columns[__index];
        assert(__column.columnType == ColumnType.Scalar);
        uint256 __columnPtr = returnPointerToTypedColumnForFixedSizeElement(__column, WORD_SIZE);
        assembly {
            __scalarColumn := __columnPtr
        }
    }
