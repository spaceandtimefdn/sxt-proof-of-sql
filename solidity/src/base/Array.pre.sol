...
library Array {
    ...
    /// ##### Signature
    /// ```yul
    /// read_uint64_array_as_wordx2_array(source_ptr) -> source_ptr_out, array_ptr
    /// ```
    /// ##### Parameters
    /// * `source_ptr` - calldata pointer to array length followed by array elements
    /// ##### Return Values
    ...
    function __readUint64ArrayAsWordx2Array(bytes calldata __source)
        external
        pure
        returns (bytes calldata __sourceOut, uint256[2][] memory __array)
    {
        assembly {
            function read_uint64_array_as_wordx2_array(source_ptr) -> source_ptr_out, array_ptr {
                array_ptr := mload(FREE_PTR)
            ...
            let __sourceOutOffset
            __sourceOutOffset, __array := read_uint64_array_as_wordx2_array(__source.offset)
            __sourceOut.offset := __sourceOutOffset
            // slither-disable-next-line write-after-write
            __sourceOut.length := sub(__source.length, sub(__sourceOutOffset, __source.offset))
        }
